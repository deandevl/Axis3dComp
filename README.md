...
1. Have `npm` installed and install the `http-server` package globally.
2. At a terminal/command window set the current directory to the `dist` folder.
...
Note that in the `index.html` of the demo it is calling the CDN's for both VUE and BABYLON so you should have a good internet connection when running this demo.
